...
| Start: Yachdal (DefineYachdel), Crowd, Crowd2 | OK | **2026-07-31: resolved by direct in-game user confirmation, corrected to `0°` for all three (was `270°`).** `270°` was reported wrong for Yachdal ("should be 90° to its left"); a static front/back render correctly showed 270° was backwards but couldn't distinguish "90° off" from "correct" or from the other 90°-off candidate — that distinction only exists from a real, moving in-game viewpoint. Two render-only guesses (`90°`, then `180°`) were both wrong per direct user testing; the user then also reported Crowd was off in the same way. All three share the same original mismeasurement, now fixed and confirmed by the user. **Genia intentionally left at `270°`** — not reported as wrong, and given this whole episode, her render-only "confirmed correct" status from earlier this session should be treated as unverified against the same left/right blind spot until someone actually checks her in-game. |
| Start: Grandma2 | OK | Flagged 2026-07-30 as unverified/absent from `mdl_yaw_allowlist.json` after a character-facing report. Checked directly (isolated render, same method as the Yachdal fix below): already correct with no allowlist entry — do not add one. |
...
| Shiks: Bus | **?** | **2026-07-28: confirmed this is a genuine data ceiling, not an unfinished investigation.** Tried the same bearing-consistency method that independently confirmed Yachdal/Crowd/Crowd2 — Bus only has 3 placements (`Menu`, `Shiks`, `golf`), each in an unrelated scene, all with `angle_gs` pan `== 0`, so there's no varying bearing to cross-check against a shared landmark. `action Bus` (`Shiks.wdl`) only plays a sound and nudges `my.y` during one cutscene trigger — no directional signal. Needs one human-confirmed measurement (screenshot or `tools/wmb_web_viewer.py`); do not guess a value. |
...
| TV / Hanger / TowerW sinking below floor | FIX | **2026-07-28.** `tools/verify_corpus.py`'s feet-snap audit re-measured every excluded stem (GLB local-Y AABB vs. WED origin + a full grep of every real WDL `action` body for `my.x/y/z` writes): all three showed the same bug shape as Cockpit (43–55% of mesh below origin, no WDL action ever moves them) and were wrongly excluded. Removed from `_should_feet_snap`. Glass/Island/BiPlane/BiPlane2/Dutyfree/B747 re-measured too and confirmed legitimately excluded (either the mesh already sits at its own floor, or a real WDL action moves the entity at runtime — see `docs/CONTRACT.md` §3.5 for the full table). Needs test. |
...
| Intro2–Intro16 (15 levels) | FIX | **2026-07-28.** Root cause of "same bugs as before" reports: all 15 marked `generic` in `LEVELS.md` — geometry loads, zero WDL ever ported, nothing scripted happens. Built a generic AST-driven WDL interpreter (`tools/parse_wdl.py` + `scripts/engine/wdl_interpreter.gd`, see `docs/CONTRACT.md` §4.1) instead of hand-porting 15 more chapters. Wired into `wdl_director.gd::_try_begin_interpreted_level()`. Multiple real bugs found and fixed via actual playtest console output (array-init crash, camera fight, `load_level` self-retrigger loop, dead `total_frames`, `float()`-on-entity-reference crashes, a corpus-wide parser bug silently dropping `Blink2`/`DoDialog` across ~55 level scripts, non-chaining includes). Still needs a real playtest of the current state. |
| Studio/Start/Town/Shiks/Plane/Plane2/Range hand-ported chapters | DELETED (2026-07-30) | **2026-07-28:** user asked to switch these from their verified hand-ported `wdl_director.gd` chapters to the generic interpreter (`HAND_PORTED` emptied, hand-port functions bypassed but left in the file). **2026-07-30:** after two more days with `HAND_PORTED` empty and no regression traced back to needing one of these chapters again, user confirmed via direct scoping questions that the dead code should be deleted outright, not left disabled — see `docs/CONTRACT.md` §4.1.3 / `docs/SESSION_LOG.md` for the full rewrite. All seven `_begin_X()`/`_update_X()` functions and their exclusive fields are gone from `wdl_director.gd`; nothing in this row's original bug list (Photo booth `morph()`, Range `CamTarget` aim/fire, Krupnik hammer/TV flipbook, TurnVase visibility, AFG cards) was reachable before the deletion either, so deleting it changed no behavior. **Still true, unchanged by tonight's rewrite: none of these 7 levels have had a human playtest through the interpreter.** Two specific checks below are new/relevant post-rewrite and still need a human pass, not just the smoke suite. |
| Rewrite regression check: Shiks (former hand-port) | Smoke-verified, not human-playtested | **2026-07-30.** `tools/smoke_click_survey.gd` confirms Shiks' interpreted level loads and its clickable entities (if any use `enable_click`) resolve with zero script errors, same as the other 15 surveyed levels. Does **not** confirm the Photo booth sequence, camera fly-through, or dialogue choices look/feel right — those need an actual playthrough, same gap as the row above. |
| Rewrite regression check: Plane's `Dome`/`Wind`/`Land` idle-spinner entities | Smoke-verified, not human-playtested | **2026-07-30.** `_update_idle_spinners()` (a second, redundant implementation of pan-jitter for these three action names, duplicating what the interpreter's own `action Dome`/`Wind`/`Land` coroutines already do when a level has a parsed AST) was deleted along with the hand-port code. `tools/smoke_dispatch.gd` confirms Plane still reaches the interpreter with these entities present. Not confirmed: whether the *visual* jitter behavior is unchanged, since that needs eyes on the running level, not just a dispatch check. |
