...
| Studio/Start/Town/Shiks/Plane/Plane2/Range hand-ported chapters | BUG (regression risk, requested) | **2026-07-28.** User explicitly asked to switch these from their verified hand-ported `wdl_director.gd` chapters to the generic interpreter, ahead of the originally-planned order (prove interpreter on untested levels + Shiks parity check first). Done via a single flag (`HAND_PORTS_ENABLED = false` in `wdl_director.gd`) — the hand-ported functions are *not deleted*, just bypassed, so flipping the flag back to `true` immediately restores the previous verified behavior. **None of these 7 levels have been tested through the interpreter at all.** Expect real regressions in the specific mechanics that took many rounds to get right this session (Shiks' Photo booth `morph()` sequence, Range's `CamTarget` aim/fire, Plane2's Krupnik hammer scrub/TV flipbook, TurnVase visibility, AFG card system) until each is re-verified the same debug-log-driven way Intro2 was. |
