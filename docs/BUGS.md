...
| GB-5 | Range: after Piposh dies, animations keep playing instead of stopping, and the retry/skip buttons that should appear don't show. | Fix shipped 2026-08-03, pending in-game confirmation. Three separate bugs: (1) the retry/map buttons on the death screen had no icon ever drawn (`_build_panel_button()` computed the texture only to size the click zone) — same gap affected every panel BUTTON in the corpus, e.g. Range's own pSkip. (2) nothing paused gameplay on death — fixed generically: the shared `pRIP` death-screen panel becoming visible now freezes every WDL coroutine's own progress at the universal per-tick `wait()` yield point, resuming exactly where it left off once hidden again; buttons stay clickable (Godot Control signals, not WDL-coroutine-driven). (3) confirmed live after (1)+(2): retry unfroze the game but didn't reset it, and a second death never showed the RIP screen again — `Death` (gates `Restart()`'s own guard) was only ever reset to 0 once, at level start; implemented the corpus-wide `level_load()` builtin (called by every level's own retry/restart path, previously an accidentally-misnamed no-op) to reset all globals to their declared initial values, matching what a real level reload does. See `docs/SESSION_LOG.md` 2026-08-03 (GB-5) and (GB-5 continued). |
| GB-7 | Range: aiming/shooting works but isn't very accurate, and the mouse/aim controls feel confusing. | Fix shipped 2026-08-04, pending in-game confirmation. User confirmed: shots don't land where the (visible) cursor points — a real aim mismatch, not a sensitivity issue. Root cause: `_enable_first_person()` is the only place `Input.mouse_mode` gets captured/hidden, but Range aims via a scripted camera (`action CamTarget` rotates `camera.pan/tilt` from raw mouse delta, `mickey.x/y`), not the first-person player path, so the OS cursor stayed visible and free-roaming — completely disconnected from the actual aim direction, and there's no rendered crosshair sprite either (Acknex's own `pan_cross_show()` isn't implemented). Fixed generically: any scripted-camera level whose WDL references `mickey` now also captures the mouse, same as first-person levels already do. See `docs/SESSION_LOG.md` 2026-08-04 (GB-7). |
...
_Last edited: 2026-08-04 (GB-7 fix shipped, pending confirmation)._
