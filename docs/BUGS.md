...
| GB-7 | Range: aiming/shooting works but isn't very accurate, and the mouse/aim controls feel confusing. | Not yet investigated in depth — checked `vec_rotate()` (the bullet's spawn-direction math) against the camera's own view-basis convention and found no obvious coordinate mismatch; likely a sensitivity/feel issue rather than a discrete bug, needs more specific reproduction detail (e.g. does the crosshair visually line up with where shots land, or does aiming feel too fast/slow/inverted). |
...
_Last edited: 2026-08-03 (GB-5 retry/Death fix shipped; GB-7 added for aim feel)._
