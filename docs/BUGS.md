...
| ID | Description | Note |
|---|---|---|
| GB-1 | Shiks: no text / animation / voice during the 2nd dialogue, regardless of which choice is picked. | Investigated 2026-08-01 (see SESSION_LOG's fifteenth/seventeenth entries) — the trigger chain, dialog choice handling, and voice playback all verified correct in every headless reproduction attempted. A related real bug *was* found and fixed (the real player's impact-trigger could double-fire and reset `DialogChoice` mid-flight, commit `7c0e36d`), but it was never confirmed as *the* cause of this specific report. Still open until confirmed fixed or re-diagnosed. |
